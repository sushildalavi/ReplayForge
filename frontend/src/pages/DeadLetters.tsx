...
function Row({ dl, refresh }: { dl: DeadLetterOut; refresh: () => void }) {
  ...
  const replay = async () => {
    ...
    try {
      ...
      setState("done");
      toast.success("Replayed", { description: `${dl.event_type} re-queued for processing` });
    ...
    <motion.tr className="tr-row"
      ...
      <td className="td">
        <Link to={`/workflows/${dl.workflow_id}`}
          className="group flex items-center gap-1 mono text-[12px] transition-colors"
          style={{ color:"#818cf8" }}
      ...
      <td className="td pr-4">
        {done ? (
          ...
          <motion.button className="btn-success" onClick={replay} disabled={state === "replaying"}
    ...
    <div className="px-6 pt-6 pb-10 space-y-5">
      ...
      {data && data.length > 0 && (
        <Stagger className="grid grid-cols-3 gap-3">
          {[
            { label:"Total DLQ", value:data.length, color:"#f1f5f9" },
            { label:"Pending replay", value:pending,  color:"#fb7185" },
            { label:"Replayed",       value:replayed, color:"#c084fc" },
          ].map(({ label,value,color }) => (
            <StaggerItem key={label}>
              <div className="card p-4 text-center">
                <p className="mono text-[24px] font-bold" style={{ color }}>{value}</p>
      ...
      {error && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-lg text-[13px]" style={{ color:"#fb7185", background:"rgba(244,63,94,0.07)", border:"1px solid rgba(244,63,94,0.18)" }}>
            ...
            {loading && !data ? [...Array(6)].map((_,i) => (
              ...
              <tr>
                <td colSpan={7} className="py-16 text-center">
                  <Skull size={28} className="mx-auto mb-2" style={{ color:"#1e2d3d" }} />
                  <p className="text-[13px]" style={{ color:"#334155" }}>No dead letters yet</p>
                  <p className="text-[12px] mt-1" style={{ color:"#1e2d3d" }}>Events that exhaust all retries appear here</p>
                </td>
              </tr>
            ) : (
              <AnimatePresence>
