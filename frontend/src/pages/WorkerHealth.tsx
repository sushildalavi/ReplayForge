...
import { Activity, AlertTriangle, Clock, Power, Server, Zap } from "lucide-react";
...
const STATUSES: Record<string, { label: string; color: string; bg: string; border: string }> = {
  active:  { label:"active",  color:"#34d399", bg:"rgba(16,185,129,0.08)",  border:"rgba(16,185,129,0.2)"  },
  busy:    { label:"busy",    color:"#fbbf24", bg:"rgba(245,158,11,0.08)",  border:"rgba(245,158,11,0.2)"  },
  stale:   { label:"stale",   color:"#fb923c", bg:"rgba(249,115,22,0.08)",  border:"rgba(249,115,22,0.2)"  },
  stopped: { label:"stopped", color:"#475569", bg:"rgba(71,85,105,0.08)",   border:"rgba(71,85,105,0.2)"   },
  crashed: { label:"crashed", color:"#f87171", bg:"rgba(239,68,68,0.08)",   border:"rgba(239,68,68,0.2)"   },
};
...
function WorkerCard({ w, i }: { w: WorkerOut; i: number }) {
  ...
  return (
    <motion.div
      ...
      transition={{ delay: i*0.06, duration:0.3, ease:[0.21,0.47,0.32,0.98] }}
      whileHover={{ y:-2, transition:{ duration:0.15 } }}
      style={{ borderColor: cfg.border }}
    >
      {/* top accent bar */}
      <div className="h-0.5 w-full" style={{ background:`linear-gradient(90deg, transparent, ${cfg.color}60, transparent)` }} />
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="mono font-semibold text-[13px] text-white">{w.worker_name}</span>
          </div>
          <span className="badge text-[11px]" style={{ background:cfg.bg, color:cfg.color, border:`1px solid ${cfg.border}` }}>
            {cfg.label}
          </span>
        </div>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[10px] uppercase tracking-[0.06em]" style={{ color:"#334155" }}>Heartbeat</span>
            <span className="mono text-[11px]" style={{ color: age > 30 ? "#fb923c" : "#475569" }}>{ago(w.last_heartbeat_at)}</span>
          </div>
          <div className="h-1.5 rounded-full overflow-hidden" style={{ background:"rgba(255,255,255,0.05)" }}>
            <motion.div className="h-full rounded-full"
              animate={isLive ? { scaleX:[1,0.2,1], opacity:[1,0.5,1] } : { scaleX: Math.max(0.05, 1 - age/60) }}
              transition={isLive ? { duration:2, repeat:Infinity, ease:"easeInOut" } : {}}
              style={{ transformOrigin:"left", background:cfg.color } as React.CSSProperties}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="card-inset px-2.5 py-2">
            ...
            <p className="mono text-[12px] font-semibold" style={{ color:cfg.color }}>{effective}</p>
          </div>
          <div className="card-inset px-2.5 py-2">
...
export default function WorkerHealth() {
  ...
  return (
    <div className="px-6 pt-6 pb-10 space-y-5">
      <FadeIn>
        <h1 className="text-[18px] font-semibold text-white flex items-center gap-2 tracking-tight">
        ...
        <Stagger className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <StaggerItem>
            <div className="card p-4 flex items-center gap-4">
              <div className="relative w-14 h-14 shrink-0">
                <svg viewBox="0 0 56 56" className="w-full h-full -rotate-90">
                  ...
                  <motion.circle cx="28" cy="28" r="22" fill="none" strokeLinecap="round"
                    ...
                    animate={{ strokeDashoffset:2*Math.PI*22*(1-healthPct/100) }}
                    transition={{ duration:1.2, ease:"easeOut" }}
                    style={{ filter:`drop-shadow(0 0 4px ${healthPct>80?"#10b981":"#f97316"}60)` }}
                  />
                </svg>
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.06em]" style={{ color:"#475569" }}>Fleet Health</p>
      ...
      {loading && !data ? (
        ...
        <FadeIn className="card p-16 text-center">
          ...
          <p className="text-[12px] mt-1" style={{ color:"#1e2d3d" }}>Start the worker service to see activity</p>
        </FadeIn>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
