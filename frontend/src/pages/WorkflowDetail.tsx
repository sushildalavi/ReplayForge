...
const TT = {
  contentStyle: { background:"#0b1120", border:"1px solid rgba(255,255,255,0.08)", borderRadius:8, fontSize:12, padding:"10px 14px" },
...
function TimelineEvent({ ev, idx, isLast }: { ev: WorkflowTimelineEventOut; idx: number; isLast: boolean }) {
  ...
  return (
    <motion.div className="flex gap-4"
      ...
      transition={{ delay: idx*0.04, duration:0.28, ease:[0.21,0.47,0.32,0.98] }}>
      ...
      <div className="flex flex-col items-center w-5 shrink-0 pt-[18px]">
        <motion.div className="w-2.5 h-2.5 rounded-full shrink-0"
          ...
          initial={{ scale:0 }}
          animate={{ scale:1 }}
          transition={{ delay:idx*0.04+0.12, type:"spring", stiffness:400, damping:20 }} />
        {!isLast && (
          <div className="w-px flex-1 mt-1" style={{ background:`linear-gradient(180deg, ${color}40 0%, rgba(255,255,255,0.03) 100%)`, minHeight:20 }} />
        )}
      </div>
      ...
      <div className="flex-1 mb-3">
        <motion.div
          ...
          animate={{ borderColor: open ? `${color}30` : "rgba(255,255,255,0.07)" }}
          transition={{ duration:0.15 }}>

          {/* header row */}
          <div className={`flex items-center gap-3 px-4 py-3 ${ev.attempts.length ? "cursor-pointer hover:bg-white/[0.018] transition-colors" : ""}`}
            onClick={() => ev.attempts.length && setOpen(!open)}>
            <div className="flex-1 flex items-center gap-2.5 min-w-0">
              <span className="mono text-[13px] font-semibold text-white truncate">{ev.event_type}</span>
              <EventStatusBadge status={ev.status} />
              <span className="text-[11px] hidden sm:block" style={{ color:"#334155" }}>{ev.service_name}</span>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              {ev.attempt_count > 1 && <span className="text-[11px] font-semibold" style={{ color:"#f97316" }}>×{ev.attempt_count} retries</span>}
          ...
          {/* error banner */}
          {ev.last_error && (
            <div className="px-4 py-2" style={{ background:"rgba(244,63,94,0.04)", borderTop:"1px solid rgba(244,63,94,0.1)" }}>
            ...
            {open && (
              <motion.div
                ...
                <div className="px-4 py-2" style={{ background:"rgba(255,255,255,0.01)" }}>
                  <p className="text-[10px] font-bold uppercase tracking-[0.08em]" style={{ color:"#1e2d3d" }}>Attempt Log</p>
                </div>
                <table className="w-full">
                  ...
                  <tbody>
                    {ev.attempts.map((a,ai) => (
                      <motion.tr key={a.id} className="tr-row"
                        ...
                        <td className="td pr-4 max-w-[200px]">
                          {a.error_message
                            ? <span className="mono text-[11px] truncate block" style={{ color:"#fb7185" }}>{a.error_message}</span>
...
export default function WorkflowDetail() {
  ...
  if (error) return <div className="px-6 pt-6 text-rose-400 text-[13px]">{error}</div>;
  if (!data) return null;
    ...
    <div className="px-6 pt-6 pb-10 space-y-5">
      ...
      <FadeIn>
        <div className="flex items-start justify-between gap-4">
          <div>
            <Link to="/" className="flex items-center gap-1 text-[12px] mb-2.5 transition-colors" style={{ color:"#334155" }}
              onMouseEnter={e => (e.currentTarget.style.color="#94a3b8")}
              onMouseLeave={e => (e.currentTarget.style.color="#334155")}>
              <ArrowLeft size={12}/> Overview
            </Link>
            <h1 className="mono text-[17px] font-semibold text-white tracking-tight">{data.workflow_id}</h1>
            <p className="text-[12px] mt-0.5" style={{ color:"#334155" }}>{total} events · {totalAttempts} attempts total</p>
          </div>
          <button className="btn-ghost" onClick={summarize} disabled={summarizing}>
      ...
      <AnimatePresence>
        {summary && (
          <motion.div
            ...
            <div className="px-4 py-2.5 flex items-center gap-2" style={{ borderBottom:"1px solid rgba(168,85,247,0.1)" }}>
              <Bot size={12} style={{ color:"#a855f7" }} />
              <p className="text-[12px] font-semibold text-white">Incident Analysis</p>
              <span className="text-[11px]" style={{ color:"#475569" }}>via {summary.model_name ?? "template"}</span>
            </div>
        ...
        {[
          ...
          <StaggerItem key={label}>
            <div className="card p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] uppercase tracking-[0.06em]" style={{ color:"#475569" }}>{label}</p>
          ...
          <div className="p-5 flex flex-col items-center gap-4">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
              {[{ l:"succeeded",v:succeeded,c:"#10b981" },{ l:"failed",v:dead,c:"#f43f5e" }].map(({ l,v,c }) => (
                <div key={l} className="text-center py-2 rounded-lg" style={{ background:"rgba(255,255,255,0.02)" }}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={barData} margin={{ top:4, right:4, left:-30, bottom:0 }} barSize={28}>
                ...
                <Bar dataKey="attempts" name="Attempts" radius={[4,4,0,0]}>
                  {barData.map((e,i) => (
                    <Cell key={i} fill={DOT[e.status]??DOT.queued}
                      style={{ filter:`drop-shadow(0 0 6px ${DOT[e.status]??DOT.queued}50)` }} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      <FadeIn delay={0.2} className="card overflow-hidden">
        ...
        <div className="p-5">
          {events.map((ev, i) => (
            <TimelineEvent key={ev.id} ev={ev} idx={i} isLast={i === events.length - 1} />
          ))}
        </div>
      </FadeIn>
    </div>
