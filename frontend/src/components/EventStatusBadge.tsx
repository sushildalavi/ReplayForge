/* animation #13 — pulse dot on processing/retrying */
...
export function EventStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="badge" style={{ background: s.bg, color: s.color, borderColor: s.border }}>
      <span className={"rounded-full shrink-0 " + (s.pulse ? "pulse-dot" : "")}
        style={{ width: 5, height: 5, background: s.dot, display: "inline-block" }} />
      {status.replace(/_/g, " ")}
    </span>
  );
}
